...
class LoginService:
    ...
    @staticmethod
    def get_new_refresh_token(request, fn):
        try:
            refresh_token = request.cookies.get(config.get("TOKENS", "REFRESHTOKEN"))
            ...
            response = fn(request)
            response.set_cookie(config.get("TOKENS", "ACCESSTOKEN"), new_access_token, max_age=int(static_variables.ACCESS_TOKEN_EXPIRE_MINUTES) * 60, httponly=True)
            response.set_cookie(config.get("TOKENS", "REFRESHTOKEN"), new_refresh_token, max_age=int(static_variables.REFRESH_TOKEN_EXPIRE_MINUTES) * 60, httponly=True)
    ...
    def login_required(role):
        def inner(fn):
            ...
            def decorator(request):
                try:
                    access_token = request.cookies.get(config.get("TOKENS", "ACCESSTOKEN"))
                    ...
                    if access_token is None:
                        ...
                        try:
                            ...
                            logger.warning(f"Access token expired for {request.cookies.get(config.get('TOKENS', 'ACCESSTOKEN'))}")
                            return redirect('/')
